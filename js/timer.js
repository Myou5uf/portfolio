...
const deadLine = "31 march 2022"; // дедлайн
...
const updateClock = () => {
    ...
    const days = Math.floor(timeRemaning / 60 / 60 / 24); // Кол-во оставшихся дней
    const hours = Math.floor((timeRemaning / 60 / 60) % 24); //  часов
    ...
    timerBlock.textContent = `${fDays}:${fHours}:${fMinutes}:${fSeconds}`; // задаем текущее для таймера
